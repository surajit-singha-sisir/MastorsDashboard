...
function dragAndDrop() {
  ...
  dropZone.addEventListener("drop", (event) => {
    ...
    handleFiles(files); // Directly handle the files for preview
  });
  ...
  fileInput.addEventListener("change", () => {
    ...
    handleFiles(files); // Handle the files when selected through the input
  });

  function handleFiles(files) {
    previewThumb(files); // Pass the files to the preview function
  }
}

// IMAGE PREVIEW AND CAPTION VISIBILITY
function previewThumb(files) {
  ...
  const file = files[0];
  if (file) {
    // Show the preview and caption sections first
    previewSection.style.display = "block"; // Show the preview section
    captionSection.style.display = "block"; // Show the caption section
...
function latestTag() {
  const latestTags = document.querySelectorAll(".latest-tag-list .latest-tags section");
...
// Initialize the functions after the DOM is loaded
document.addEventListener("DOMContentLoaded", function () {
  tags();
  latestTag();
});
